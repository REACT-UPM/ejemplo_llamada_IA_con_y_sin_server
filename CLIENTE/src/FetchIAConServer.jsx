...
const endpoint = 'http://localhost:3001/recomendacion';

export default function FetchIA({ }) {
  const genres = [
        'Fantasía',
        'Ciencia ficción',
        'Misterio',
  ...
  // El cliente ahora llama a nuestro servidor /recomendacion para no exponer la API key
  const onSubmit = async (e) => {
    ...
    try {
        ...
        if (data.success) {
          ...
          setError(data.error || 'No se pudo obtener una recomendación. Intenta de nuevo.');
        }
    } catch (err) {
